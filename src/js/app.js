...
function populatePage(data) {
    ...
    if (servicesSection && data.services) {
        ...
        if (servicesContainer) {
            servicesContainer.innerHTML = data.services.items
                .map(
                    (service) => `
    ...
    if (insightsSection && data.insights) {
        ...
        if (insightsContainer) {
            insightsContainer.innerHTML = data.insights.items
                .map(
                    (insight) => `
    ...
    if (data.sharedData) {
        ...
        `;
                const target =
                    contactSection.querySelector(".max-w-2xl") ||
                    contactSection;
                target.appendChild(contactInfo);
            }
            // Se houver campos explícitos em páginas legais, atualiza
            const email = data.sharedData.contact.email || "";
            const phone = data.sharedData.contact.phone || "";

            // Texto
            const emailEl = document.getElementById("contact-email");
            if (emailEl) emailEl.textContent = email;

            const privacyEmailEl = document.getElementById(
                "privacy-contact-email",
            );
            if (privacyEmailEl) privacyEmailEl.textContent = email;
        ...
        if (data.sharedData.companyInfo) {
            ...
            if (nameEl) nameEl.textContent = ci.name || nameEl.textContent;
            const cnpjEl = document.getElementById("company-cnpj");
            if (cnpjEl) cnpjEl.textContent = ci.cnpj || cnpjEl.textContent;
            const addrEl = document.getElementById("company-address");
            if (addrEl) addrEl.textContent = ci.address || addrEl.textContent;
        }
        // Document meta (última atualização / versão) — procura em sharedData.meta
        const meta =
            (data.sharedData && data.sharedData.meta) ||
            data.meta ||
            (data.footer && data.footer.meta);
        if (meta) {
            const lastEl = document.getElementById("doc-last-updated");
            if (lastEl && meta.lastUpdated)
                lastEl.textContent = meta.lastUpdated;
            const verEl = document.getElementById("doc-version");
            if (verEl && meta.version) verEl.textContent = meta.version;
        }
    }
    ...
    if (footer && data.footer) {
        ...
        if (footerTextEl)
            footerTextEl.textContent =
                data.footer.copyright || footerTextEl.textContent;
        const footerTaglineEl = document.getElementById("footer-tagline");
        if (footerTaglineEl)
            footerTaglineEl.textContent =
                data.footer.tagline || footerTaglineEl.textContent;

        // LINKS PARA PÁGINAS LEGAIS - FIXADO
        const legalLinksContainer = document.createElement("div");
        legalLinksContainer.className =
            "mt-6 flex flex-wrap justify-center gap-4 md:gap-6";
    ...
    `;
        ...
        if (
            data.sharedData &&
            data.sharedData.social &&
            Array.isArray(data.sharedData.social)
        ) {
...
function showErrorState() {
    const mainElements = document.querySelectorAll(
        "#hero-title, #hero-subtitle, #about-text",
    );
    mainElements.forEach((el) => {
        if (el)
            el.textContent =
                "Falha ao carregar conteúdo. Por favor, atualize a página.";
    });
}
